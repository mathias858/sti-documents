...
import { Inbox as InboxIcon, Eye, CornerUpRight, Download, Search } from "lucide-react";
...
export default function SearchFiles() {
  ...
  const { data: documents, isLoading } = useGetDocuments({ search: debouncedSearch });
  ...
  return (
    <AppLayout title="">
      <div className="animate-in fade-in duration-500 space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Search Files</h1>
          <p className="text-slate-500 mt-1">Locate documents across the entire registry</p>
        </div>
        ...
        <Card className="border-slate-200 shadow-sm overflow-hidden bg-white">
          <div className="p-4 border-b border-slate-100 bg-white flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input 
                placeholder="Search by subject, reference number, or keywords..." 
                className="pl-9 py-6 text-lg bg-slate-50 border-slate-200 w-full" 
                value={searchQuery}
          ...
          {isLoading ? (
            <div className="p-12 text-center text-slate-400">Searching directory...</div>
