...
import { Archive as ArchiveIcon, Eye, Download, Search } from "lucide-react";
...
export default function Archive() {
  const { data: documents, isLoading } = useGetDocuments({ status: 'SIGNED,CLOSED' });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'CLOSED':
        return <Badge variant="secondary" className="bg-slate-600 text-white hover:bg-slate-700">CLOSED</Badge>;
      default:
  ...
  return (
    <AppLayout title="">
      <div className="animate-in fade-in duration-500 space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Archive</h1>
          <p className="text-slate-500 mt-1">Closed and fully processed documents</p>
        </div>
        ...
        <Card className="border-slate-200 shadow-sm overflow-hidden bg-white">
          <div className="p-4 border-b border-slate-100 bg-white flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search archive by reference..." className="pl-9 bg-slate-50 border-slate-200 w-full" />
            </div>
            <div className="flex gap-2">
              <Select defaultValue="all_years">
          ...
          {isLoading ? (
              ...
              <Table>
                <TableHeader className="bg-slate-50">
                  <TableRow>
                    ...
                    <TableHead className="font-semibold text-slate-600 text-xs uppercase tracking-wider">Date Closed</TableHead>
                  ...
                  {documents.map((doc) => (
                    <TableRow key={doc.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <TableCell className="text-slate-600 cursor-pointer" onClick={() => setSelectedDocId(doc.id)}>
                        {doc.originator.department.name}
                      </TableCell>
            ...
            <div className="p-16 text-center">
              <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-100">
                <ArchiveIcon className="w-10 h-10 text-slate-300" />
              </div>
              <h3 className="text-lg font-medium text-slate-900">Archive Empty</h3>
              <p className="text-slate-500 mt-1">There are no closed documents in the system yet.</p>
            </div>
          )}
        </Card>
      </div>
